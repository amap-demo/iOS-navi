...
class CustomTurnInfoViewController: UIViewController, AMapNaviDriveManagerDelegate, AMapNaviDriveViewDelegate, AMapNaviDriveDataRepresentable {
    ...
    func driveManager(_ driveManager: AMapNaviDriveManager, update cameraInfos: [AMapNaviCameraInfo]?) {
        guard let allInfos = cameraInfos else {
            cameraInfoLabel.text = "电子眼信息:暂无"
            return
        }
        ...
        //距离最近的下个电子眼信息
        if let camera = allInfos.first {
            if camera.distance > 0 {
                cameraStr = camera.cameraType == .speed ? String(format: "测速(%d)", camera.cameraSpeed) : "监控"
            }
        }
